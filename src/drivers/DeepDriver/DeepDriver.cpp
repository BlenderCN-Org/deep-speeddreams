...
static int const sNumberOfDrivers = 1;
static CDeepDriver * spDriver[sNumberOfDrivers];
 ...
 * Module entry point  
 */ 
extern "C" int DeepDriver(tModInfo *modInfo)
{
  memset(modInfo, 0, MAX_MOD_ITF*sizeof(tModInfo));

  std::string Description = "Is driven by the network.";

  for(int i = 0; (i < MAX_MOD_ITF) && (i < sNumberOfDrivers); i++)
  {
    std::string const Name = "Deep Driver";

    assert(spDriver[i] == NULL);
    spDriver[i] = new CDeepDriver(Name.c_str(), Description.c_str());
    assert(spDriver[i] != NULL);
